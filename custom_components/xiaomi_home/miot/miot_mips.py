...
class MipsClient(ABC):
    ...
    def __init__(
        ...
        self._cmd_event_fdr, self._cmd_event_fdw = os.pipe()
        self.mev_set_read_handler(
            self._cmd_event_fdr, self.__mips_cmd_read_handler, None)
    ...
    def _mips_send_cmd(self, type_: MipsCmdType, data: any) -> bool:
        if self._mips_queue is None or self._cmd_event_fdw is None:
            raise MIoTMipsError('send mips cmd disable')
        # Put data to queue
        self._mips_queue.put(MipsCmd(type_=type_, data=data))
        # Write event fd
        os.write(self._cmd_event_fdw, '1'.encode())
    ...
    def __mips_cmd_read_handler(self, ctx: any) -> None:
        fd_value = os.read(self._cmd_event_fdr,1)
        if fd_value == 0:
            return
        while self._mips_queue.empty() is False:
            ...
            if mips_cmd.type_ == MipsCmdType.CONNECT:
                ...
                self.__mips_disconnect()
                # Close cmd event fd
                if self._cmd_event_fdr:
                    self.mev_set_read_handler(
                        self._cmd_event_fdr, None, None)
                    os.close(self._cmd_event_fdr)
                    self._cmd_event_fdr = None
                if self._mips_queue:
